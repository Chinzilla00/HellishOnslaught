                ...
                tasks.Insert(Altar + 1, new PassLegacy("Generating Quarry Altar", delegate (GenerationProgress progress)
                {
                    ...
                    while (!placed)
                    {
                        ...
                        if (Main.wallDungeon[Main.tile[PlaceHere, PlacementHeight].wall] && !Main.tile[PlaceHere, PlacementHeight].active())
                        {
                            ...
                            {
                                PlacementHeight++;
                                if (WorldGen.SolidTile(PlaceHere, PlacementHeight) && ((Main.tile[PlaceHere - 1, PlacementHeight - 1].type == TileID.BlueDungeonBrick && Main.tile[PlaceHere, PlacementHeight - 1].type == TileID.BlueDungeonBrick && Main.tile[PlaceHere + 1, PlacementHeight - 1].type == TileID.BlueDungeonBrick) || (Main.tile[PlaceHere - 1, PlacementHeight + 1].type == TileID.GreenDungeonBrick && Main.tile[PlaceHere, PlacementHeight - 1].type == TileID.GreenDungeonBrick && Main.tile[PlaceHere + 1, PlacementHeight - 1].type == TileID.GreenDungeonBrick) || (Main.tile[PlaceHere - 1, PlacementHeight - 1].type == TileID.PinkDungeonBrick && Main.tile[PlaceHere, PlacementHeight - 1].type == TileID.PinkDungeonBrick && Main.tile[PlaceHere + 1, PlacementHeight - 1].type == TileID.PinkDungeonBrick)))
                                {
